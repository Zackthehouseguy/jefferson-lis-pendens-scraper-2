...
def main()->int:
    ...
    for ranked in hr.get('ranked_live_leads') or []:
        if int(ranked.get('priority_score') or 0)<MIN_PRIORITY:
            house_reject.append({'case_number':ranked.get('case_number'),'reason':'priority_below_60','priority':ranked.get('priority_score')});continue
    ...
    for ranked in lr.get('ranked_land') or []:
        if int(ranked.get('priority_score') or 0)<MIN_PRIORITY: continue
    ...
    report={'status':status,'minimum_priority':MIN_PRIORITY,'target_each':TARGET,'houses_count':len(houses),'land_count':len(land),
            ...
            'houses':houses,'land':land,'house_rejections':house_reject,
            'guardrails':{'sfr_requires_lojic_landuse_single_family':True,'priority_must_be_at_least_60':True,'multifamily_not_allowed_in_house_lane':True,
    ...
    print(json.dumps({'status':status,'houses':len(houses),'land':len(land),'house_rejections':len(house_reject)},indent=2))
